...
# === 关键修正: 损失函数 ===
class WeightedPhaseAwareLoss(nn.Module):
    def __init__(self, alpha=0.5, amp_weight=20.0): # amp_weight: 光斑区域的额外权重
        super().__init__()
        self.alpha = alpha
        self.amp_weight = amp_weight
        self.l1 = nn.L1Loss(reduction='none') # 改为 none，以便我们手动加权
    ...
    def forward(self, pred, target):
        ...
        # 1. 振幅损失 (带空间加权)
        # 创建权重掩膜：如果目标振幅 > 0.1 (是有光斑的地方)，权重 x20，否则 x1
        spatial_weight = torch.ones_like(target_amp)
        spatial_weight[target_amp > 0.1] = self.amp_weight
        
        loss_amp_pixel = self.l1(pred_amp, target_amp)
        loss_amp = torch.mean(loss_amp_pixel * spatial_weight) # 加权平均
        
        # 2. 相位损失 (仅在振幅足够强的地方计算相位，避免背景噪声干扰相位训练)
        # 只有当 target_amp 大于阈值时，相位才重要。背景的相位其实无所谓。
        diff = pred_phase - target_phase
        loss_phase_pixel = 1 - torch.cos(diff)
        
        # 使用 target_amp 作为相位的自然权重
        # 意味着：越亮的地方，相位越要准；黑的地方，相位无所谓
        loss_phase = torch.mean(loss_phase_pixel * (target_amp + 0.1)) 
        
        return loss_phase + self.alpha * loss_amp, loss_amp, loss_phase

def train(model, loader, val_loader, device, epochs=50):
    model.to(device)
    criterion = WeightedPhaseAwareLoss(alpha=1.0) # 振幅和相位权重
    ...
    torch.save(model.state_dict(), "./ustc/Others/ai原子阵列重排/training_result/training_result_05/best_model_fixed.pth")
...
def infer_and_visualize(model, val_loader, device):
    ...
    plt.savefig("./ustc/Others/ai原子阵列重排/training_result/training_result_05/inference_result_fixed.png")
    print("Result saved.")
