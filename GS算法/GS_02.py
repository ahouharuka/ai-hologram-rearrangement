...
class WeightedGSAlgorithm:
    ...
    def calculate_error(self, focal_field):
        """计算当前迭代的误差"""
        current_amplitude = np.abs(focal_field)
        error = np.sqrt(np.sum(self.weights * (current_amplitude - self.target_amplitude)**2))
...
def create_weights(size, traps_positions, spot_weight=0.98, background_weight=0.2):
    """
    创建权重矩阵，强调光镊位置的重要性
    ...
    for pos in traps_positions:
        ...
        # 创建一个以光斑为中心的高斯权重分布，峰值在 spot_weight
        # 衰减到 background_weight
        gaussian_weights = background_weight + (spot_weight - background_weight) * np.exp(-distance**2 / (2 * 3**2))
        # 取最大值，确保光斑区域的高权重得以保留
        weights = np.maximum(weights, gaussian_weights)
...
if __name__ == "__main__":
    # 设置参数
    slm_size = (256, 256)  # SLM分辨率
    iterations = 100  # 迭代次数
    
    # 创建目标光镊阵列（例如5x5网格）
    traps_positions = []
    grid_size = 5
    spacing = slm_size[0] // (grid_size + 1)
    ...
    # 创建权重矩阵（强调光镊位置）
    weights = create_weights(slm_size, traps_positions, spot_weight=0.7)
    ...
    # reconstructed_amplitude = np.ones(slm_size)*1000 - np.abs(focal_field)
    
    # 可视化结果
    visualize_results(target_amplitude, hologram, reconstructed_amplitude, errors)
    print(target_amplitude, focal_field)
