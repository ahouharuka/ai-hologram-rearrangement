...
def generate_training_samples(num_samples, output_dir, slm_size=(1024, 1024), real_slm_size=(256, 256), iterations=200, min_traps=5, max_traps=50):
    ...
    for sample_idx in range(num_samples):
        ...
        try:
            # 1. 随机生成光镊配置
            traps_positions = generate_trap_configuration(slm_size, real_slm_size, min_traps, max_traps)
            
            print(f"生成 {len(traps_positions)} 个光镊")
            ...
            if fidelity > 0.85:  # 设置质量阈值
                ...
                sample_data = {
                    ...
                    'num_traps': np.array([len(traps_positions)], dtype=np.int32),
                ...
                if successful_samples % 10 == 0:
                    ...
                    axes[0].set_title(f'Target ({len(traps_positions)} traps, Fidelity: {fidelity:.3f})')
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = "gs_training_data/gs_training_data_02"  # 输出目录
    MIN_TRAPS = 5
    MAX_TRAPS = 50
